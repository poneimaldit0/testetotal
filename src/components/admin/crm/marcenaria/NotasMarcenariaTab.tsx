...
import { useNotasMarcenaria } from "@/hooks/useNotasMarcenaria";
...
interface NotasMarcenariaTabProps {
  leadId: string;
}

export function NotasMarcenariaTab({ leadId }: NotasMarcenariaTabProps) {
  const { profile } = useAuth();
  const { notas, isLoading, adicionarNota, isAdicionando, editarNota, deletarNota } = useNotasMarcenaria(leadId);
  ...
  const handleIniciarEdicao = (nota: NotaMarcenaria) => {
  ...
  const podeEditar = (nota: NotaMarcenaria) => {
