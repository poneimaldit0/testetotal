-- Recriar view_orcamentos_crm_com_checklist incluindo contadores de tarefas
...
SELECT 
  ...
  tarefas.tem_hoje,
  -- Contadores de tarefas do CRM (restaurados)
  COALESCE(tarefas_count.total_tarefas, 0)::INTEGER AS total_tarefas,
  COALESCE(tarefas_count.tarefas_concluidas, 0)::INTEGER AS tarefas_concluidas,
  COALESCE(tarefas_count.tarefas_atrasadas, 0)::INTEGER AS tarefas_atrasadas,
  COALESCE(tarefas_count.tarefas_hoje, 0)::INTEGER AS tarefas_hoje,
  -- Dados do gestor
...
LEFT JOIN LATERAL (
  SELECT 
    ...
    COUNT(*) FILTER (WHERE concluida = false AND data_vencimento < CURRENT_DATE) as tarefas_atrasadas,
    COUNT(*) FILTER (WHERE concluida = false AND data_vencimento = CURRENT_DATE) as tarefas_hoje
...
GROUP BY 
  ...
  tarefas_count.total_tarefas, tarefas_count.tarefas_concluidas,
  tarefas_count.tarefas_atrasadas, tarefas_count.tarefas_hoje,
  p.nome, p.email;