-- Corrigir função processar_candidatura_fornecedor removendo FOR UPDATE do COUNT
-- que estava causando erro "FOR UPDATE is not allowed with aggregate functions"
...
BEGIN
  ...
  -- Buscar orçamento (SEM FOR UPDATE para evitar erro de permissão)
  ...
  -- Contar candidatos atuais (SEM FOR UPDATE para evitar erro com função agregada)
  ...
  IF candidatos_atuais >= 3 THEN
    -- Tentar atualizar status do orçamento (pode falhar silenciosamente se sem permissão)
    BEGIN
    ...
    EXCEPTION
      WHEN OTHERS THEN
        NULL; -- Ignorar erro de permissão no UPDATE
    END;
