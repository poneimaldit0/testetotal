...
USING (
  ...
  EXISTS (
    SELECT 1 FROM public.diario_obra do
    WHERE do.fornecedor_id = auth.uid()
    AND storage.foldername(name)[1] = do.fornecedor_id::text
    AND storage.foldername(name)[2] = do.contrato_id::text
  )
);
...
CREATE POLICY "Fornecedores podem atualizar fotos de seus contratos"
...
USING (
  ...
  EXISTS (
    SELECT 1 FROM public.diario_obra do
    JOIN public.contratos c ON c.id = do.contrato_id
    JOIN public.clientes cl ON cl.id = c.cliente_id
    WHERE cl.auth_user_id = auth.uid()
    AND do.visivel_cliente = true
    AND storage.foldername(name)[1] = do.fornecedor_id::text
    AND storage.foldername(name)[2] = do.contrato_id::text
  )
);
