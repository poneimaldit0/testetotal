-- Atualiza a função de relatório de funil acumulado para filtrar por data de ENTRADA no CRM
-- (primeira movimentação onde etapa_anterior IS NULL) em vez de data de cada movimentação
...
BEGIN
  ...
  WITH leads_no_periodo AS (
    ...
    WHERE och.etapa_anterior IS NULL  -- Primeira movimentação = entrada no CRM
      ...
      AND (gestor_id IS NULL OR oct.gestor_conta_id = gestor_id)
      AND is_admin()
  ),
  contagem_etapas AS (
