...
-- Recriar a view com a correção no cálculo de alertas
...
SELECT 
    ...
    -- CORREÇÃO: Usar ccp.created_at em vez de oct.updated_at para calcular alertas
    (EXISTS ( SELECT 1
           FROM crm_checklist_progresso ccp
             JOIN crm_checklist_etapas cce ON cce.id = ccp.item_checklist_id
          WHERE ccp.orcamento_id = o.id AND ccp.concluido = false AND cce.etapa_crm = oct.etapa_crm AND (now() - ccp.created_at) >= (cce.dias_para_alerta::double precision * '1 day'::interval))) AS tem_alertas,
