-- Criar função helper para obter a data atual no fuso horário de São Paulo
...
-- Atualizar função processar_candidatura_fornecedor para usar timezone de São Paulo
CREATE OR REPLACE FUNCTION public.processar_candidatura_fornecedor(p_orcamento_id uuid, p_nome text, p_email text, p_telefone text, p_empresa text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO ''
...
BEGIN
  ...
  -- CORREÇÃO: Contar candidaturas do dia usando timezone de São Paulo
  SELECT COUNT(*) INTO candidaturas_hoje
  FROM public.candidaturas_fornecedores
  ...
  IF user_record.limite_acessos_diarios IS NOT NULL AND 
     candidaturas_hoje >= user_record.limite_acessos_diarios THEN
    RETURN jsonb_build_object(
      'success', false,
  ...
  -- CORREÇÃO: Contar candidaturas do mês usando timezone de São Paulo
  SELECT COUNT(*) INTO candidaturas_mes
  FROM public.candidaturas_fornecedores
  WHERE fornecedor_id = auth.uid()
    AND DATE_TRUNC('month', data_candidatura AT TIME ZONE 'America/Sao_Paulo') = 
        DATE_TRUNC('month', (NOW() AT TIME ZONE 'America/Sao_Paulo'));
  
  -- Verificar limites mensais
  IF user_record.limite_acessos_mensais IS NOT NULL AND 
     candidaturas_mes >= user_record.limite_acessos_mensais THEN
    RETURN jsonb_build_object(
      'success', false,
  ...
  SET acessos_diarios = candidaturas_hoje + 1,
      acessos_mensais = candidaturas_mes + 1,
      ultimo_login = now()
  WHERE id = auth.uid();
