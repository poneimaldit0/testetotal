-- Criar função para relatório de perfil de orçamentos (versão final)
...
estatisticas_tamanho AS (
  SELECT 
    COUNT(*) as total,
    ROUND(AVG(tamanho_imovel)::numeric, 2) as media,
    ROUND(PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY tamanho_imovel)::numeric, 2) as mediana,
...
total_categorias AS (
  SELECT SUM(quantidade) as total_cat FROM categorias_stats
),
top_categorias AS (
  SELECT jsonb_agg(
    jsonb_build_object(
      ...
      'percentual', ROUND((cs.quantidade * 100.0 / tc.total_cat)::numeric, 2)
    ) ORDER BY cs.quantidade DESC
  ) as categorias_json
