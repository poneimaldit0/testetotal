-- Criar trigger para inicializar checklist colaborativo automaticamente
...
BEGIN
  ...
  IF checklist_existente = 0 THEN
    INSERT INTO public.checklist_colaborativo (
      ...
      NEW.orcamento_id,
      'aguardando_primeiro_preenchimento',
      1,
      0
    );
...
-- Criar trigger na tabela candidaturas_fornecedores
DROP TRIGGER IF EXISTS trigger_inicializar_checklist_colaborativo ON public.candidaturas_fornecedores;
CREATE TRIGGER trigger_inicializar_checklist_colaborativo
  AFTER INSERT ON public.candidaturas_fornecedores
  FOR EACH ROW
  EXECUTE FUNCTION public.inicializar_checklist_colaborativo();

-- Função para criar checklists colaborativos retroativamente
