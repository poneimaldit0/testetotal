-- Migration: Corrigir race condition em candidaturas de fornecedores
-- Data: 2025-01-10
-- Descrição: Adiciona lock pessimista para garantir que apenas 3 fornecedores possam se candidatar

-- Remover ambas as versões da função existente
DROP FUNCTION IF EXISTS public.processar_candidatura_fornecedor(uuid, text, text, text, text);
DROP FUNCTION IF EXISTS public.processar_candidatura_fornecedor(uuid, uuid, text, text, text, text);
...
BEGIN
  ...
  -- Fazer lock no orçamento ANTES de contar candidatos
  SELECT * INTO orcamento_record
  FROM public.orcamentos
  WHERE id = p_orcamento_id
  FOR UPDATE;  -- Lock exclusivo até o fim da transação
...
COMMENT ON FUNCTION public.processar_candidatura_fornecedor IS 
'Processa candidatura de fornecedor com lock pessimista para prevenir race conditions e garantir limite de 3 candidatos';