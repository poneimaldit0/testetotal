-- Atualizar função buscar_proposta_por_codigos para incluir dados completos da proposta
...
BEGIN
  ...
  -- Buscar dados do checklist da proposta
  SELECT jsonb_build_object(
    ...
    'categorias', COALESCE(
      (
        ...
        JOIN public.checklist_itens ci ON ci.categoria = categoria_itens.categoria
        GROUP BY ci.categoria
      ),
      '{}'::jsonb
    )
