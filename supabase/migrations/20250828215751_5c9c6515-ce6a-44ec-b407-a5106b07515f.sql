...
-- Focando apenas nas tabelas que realmente existem

-- Primeiro, dropar a constraint existente do checklist_colaborativo e recriar com CASCADE
...
-- Contas a receber (SET NULL em vez de CASCADE para preservar dados financeiros)
...
-- Avaliações de fornecedores (SET NULL em vez de CASCADE para preservar avaliações)
...
VALUES (auth.uid(), 'correcao_constraints_exclusao_orcamentos_v2');