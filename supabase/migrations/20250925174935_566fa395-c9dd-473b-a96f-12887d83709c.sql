-- Ajustar função relatorio_lt_churn para permitir testes e melhorar debugging
...
BEGIN
  ...
  -- Se tem usuário logado, verificar tipo
  IF current_user_id IS NOT NULL THEN
    ...
    IF user_tipo IN ('admin', 'master', 'gestor_conta') THEN
      has_permission := TRUE;
    END IF;
  ELSE
    -- Para testes diretos, verificar se existe pelo menos um admin/master ativo
    SELECT EXISTS(
      SELECT 1 FROM public.profiles p 
      WHERE p.tipo_usuario IN ('admin', 'master') 
        AND p.status = 'ativo'
    ) INTO has_permission;
  END IF;
  ...
  IF NOT has_permission THEN
    RAISE EXCEPTION 'Acesso negado. Usuário sem permissão para acessar relatórios. User ID: %, Tipo: %', 
      COALESCE(current_user_id::text, 'NULL'), COALESCE(user_tipo, 'NULL');
  END IF;
  ...
  SELECT 
    ...
    ROUND((mg.churn_rate_periodo / 12.0)::NUMERIC, 2) as churn_rate_mensal,
    -- Dados das coortes
    (
      SELECT jsonb_agg(
        jsonb_build_object(
