-- Corrigir status_contrato para considerar status do perfil
DROP FUNCTION IF EXISTS relatorio_experiencia_fornecedor();
...
WITH dados_fornecedor AS (
  SELECT 
    ...
    COUNT(DISTINCT CASE WHEN o.status = 'aberto' THEN ins.orcamento_id END) as orcamentos_abertos,
    COUNT(DISTINCT CASE WHEN cp.status = 'enviada' THEN cp.id END) as propostas_enviadas,
  ...
  FROM profiles p
  LEFT JOIN inscricoes_fornecedores ins ON p.id = ins.fornecedor_id
  LEFT JOIN orcamentos o ON ins.orcamento_id = o.id
  LEFT JOIN candidaturas_fornecedores cf ON p.id = cf.fornecedor_id
  LEFT JOIN checklist_propostas cp ON cf.id = cp.candidatura_id
...
fornecedor_com_metricas AS (
  SELECT 
    ...
    CASE
      -- Se o usuário não está ativo, não pode ter contrato ativo
      WHEN df.status_perfil != 'ativo' THEN 
        CASE 
          ...
          ELSE 'inativo'
        END
      -- Se o usuário está ativo, verificar a data do contrato
...
SELECT 
  ...
  CASE
    ...
    ELSE 'ok'
  END as nivel_alerta,
  calcular_gatilhos_ativos(fm.dias_inativo, fm.orcamentos_abertos, fm.dias_plataforma) as gatilhos_ativos,
  definir_acao_sugerida(fm.dias_inativo, fm.orcamentos_abertos, fm.dias_plataforma, fm.nome) as acao_sugerida,
  calcular_prioridade(fm.dias_inativo, fm.orcamentos_abertos, fm.dias_plataforma, fm.status_contrato) as prioridade
