-- Corrigir função relatorio_lt_churn removendo verificação restritiva de admin
...
DECLARE
  ...
  total_dias INTEGER;
BEGIN
  -- Verificar se o usuário tem permissão (mais flexível)
  IF NOT (public.is_admin() OR public.is_gestor_conta() OR public.is_master()) THEN
    RAISE EXCEPTION 'Acesso negado. Usuário sem permissão para acessar relatórios.';
  END IF;
  ...
  SELECT 
    ...
    30.0::NUMERIC as churn_rate_mensal, -- Estimativa baseada no período
    -- Dados das coortes
    (
      SELECT jsonb_agg(
        jsonb_build_object(
      ...
      'faixas', (
        SELECT jsonb_agg(
          jsonb_build_object(
            ...
            'percentual', ROUND(
              (fd.quantidade * 100.0 / mg.total_fornecedores)::NUMERIC, 2
            )
          )
        )
