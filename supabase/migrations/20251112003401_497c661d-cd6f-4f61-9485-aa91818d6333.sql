...
-- Recriar a view adicionando a coluna tags (usando JSONB)
...
SELECT 
    ...
    -- Agregação de tags (NOVO - usando JSONB)
    ...
    (EXISTS ( SELECT 1
          ...
          WHERE ccp.orcamento_id = o.id AND ccp.concluido = false AND cce.etapa_crm = oct.etapa_crm AND (now() - oct.updated_at) >= (cce.dias_para_alerta::double precision * '1 day'::interval))) AS tem_alertas,
...
-- Agregação de tags (NOVO - usando JSONB_AGG)
LEFT JOIN LATERAL (
    SELECT jsonb_agg(
        jsonb_build_object(
            'id', tags.id,
